...
```{r}
# Party registrations
Party_Registration <- read_csv("data/Tabula-Party_Registration-2024_09.csv", col_names = FALSE)
...
```{r}
Clean_Party_Registration <- Party_Registration |>
  slice(-c(1, 2)) |>
  rename(
    Registration_Type = X1,
    DEM = X2,
    REP = X3,
    NLM = X4,
    LIB = X5,
...
```{r}
Clean_Party_Registration |>
  mutate_if(is.character, 
            ~ as.numeric(gsub(",", "", .)))

#I had to search online for how to remove the first two rows and convert commas to decimal points. I tried various methods we previously covered, but none of them worked as expected.I would like to review on these.   
```
