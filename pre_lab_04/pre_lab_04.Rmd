...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** Several recipients with the same name are listed differently due to spelling mistakes. 
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** "W. Millanr & co" in recipient_clean became all in capital letters. The same happened to "W Millar Catering", which in recipient_clean also added a "." after the "W". In addition,"Walmart" became "WALMART". 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** There is no a big change from the previous one. Again, here recipient_name that are in lowercase became uppercase in recipient_clean. 
