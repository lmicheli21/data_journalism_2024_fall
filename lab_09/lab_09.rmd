...
**A1.** https://datawrapper.dwcdn.net/TcOy9/1/ 

Interestingly, this data shows that, despite the pandemic's decline, the percentage of mail voting remained significantly higher in 2022 than pre-pandemic levels. It increased from 7.1& in 2018 to 19.3% in 2022. This trend might change for ever the ways electors vote.


#how the methods of voting have changed in Maryland since 2018
```{r}
md_votes_by_type <- read_csv("data/md_votes_by_type.csv") |> clean_names()

```
```{r}
#the percentage of votes cast in each category - early voting, election day and mail - over the three years
...
```{r}
md_votes_by_type_better <- md_votes_by_type |>
  select(year, pct_early, pct_election_day, pct_mail)


write_csv(md_votes_by_type_better, "data/md_votes_by_type_better.csv")
```
...
**A2.** https://datawrapper.dwcdn.net/OSXyf/2/

Is there an age-based preference for early voting?

I compared these two datasets to explore whether voting preferences, particularly for early voting, differ by age group.
Overall, this area graph highlights a notable preference for early voting among older voters compared to younger voters in Maryland. Focusing on Montgomery, Prince George's, and Baltimore Counties, the graph reveals a consistent trend of higher early voting rates among older residents across multiple days.

```{r}
ev_totals_18_24 <- read_csv("data/ev_totals_18_24.csv") |> clean_names()
ev_totals_65_up <- read_csv("data/ev_totals_65_up.csv") |> clean_names()
```


```{r}
ev_comparison <- ev_totals_18_24 |> 
  inner_join(ev_totals_65_up, by = 'day')
```


```{r}

ev_comparison_selected <- ev_comparison |>
  select(day,montgomery.y, montgomery.x, prince_georges.x, prince_georges.y, baltimore_county.x, baltimore_county.y) |>
  rename(montgomery18_24 = montgomery.x,
         montgomery65 = montgomery.y,
         prince_georges18_24 = prince_georges.x,
         prince_georges65 = prince_georges.y,
         baltimore_county18_24 = baltimore_county.x,
         baltimore_county65 = baltimore_county.y)

write_csv(ev_comparison_selected, "data/ev_comparison_selected.csv")
