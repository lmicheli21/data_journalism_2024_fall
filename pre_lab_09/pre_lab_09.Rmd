...
**Answer** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url. https://datawrapper.dwcdn.net/MS7th/1/ 
...
md_gov_county <- md_gov_county |>
  mutate(
    total = Cox + Moore + Lashar + Wallace + Harding + `Write-ins`,
    pct_moore = (Moore / total) * 100
  )

write_csv(md_gov_county, "data/md_gov_county_better.csv")
```
...
Now we finally have a well detailed map that shows the percentage of votes received by Wes Moore in each county of Maryland. The labels and colours are now right. 
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **Answer** Put the URL of your map here. https://datawrapper.dwcdn.net/wh63P/1/ 
