...
**Answer** it makes it more readable. Moltiplying for 100 makes us reading the actual percentage instead of a decimal digit. 
...
**Answer** we can order the percentage results from the biggest to the smallest. 
...
**Answer** at first sight, it simply gives us another point of view. However, when we come to important names, such as Biden, it is interesting to see how many people in percentages voted for these. 
...
**Answer** 
1. str_to_title() converts to title case, where only the first letter of each word is capitalized.
2. str_to_sentence() convert to sentence case, where only the first letter of sentence is capitalized.
...
**Answer** In this case, the mutate statement is creating a new column and putting in a category value based on the amount of each record. Then, case_when introduces  a conditional filter. Overall, the code notes if the amount is less than $100, between $100 and $499, between $500 and $1499, between $500 and $2999, and $3000 or more. 
...
**Answer** the largest category in terms of dollar amount is "less than $100". 
...
**Answer** select is useful in order to simplify our dataset. For instance, in this case we  only want to look at the office district, candidate name, party and votes. In fact, selecting simply returns only the fields named. 
...
**Answer:** now is no more a <chr> but it is <date>. No more with '/' but with '-'
...
**Answer** the data is showing the total amount of expenses in Maryland for each month. 
