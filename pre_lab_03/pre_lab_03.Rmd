...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer**  There are 1640 with this problem. R is showing that the .Csv shows figures in a character column. 
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No. It doesn't show any problems. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** district is "dbl", precint is "chr", and mail is "dbl". 
...
**Answer** The code has just changed where it was written '5+' in '49'. Moreover, it converted this data from being a <chr> to a <dbl>.  
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: NA are 402345. It means that there is an error when in Texast they tried to count votes by mail, umproperly listing them. 
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** For instance, it is interesting to take a look at the "deceased" row as it represents a large number of people. Probably, it is about people that were available to vote but died in the voting time. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** it is shown as <chr>. 
...
**Answer** The clean_names() function serves to replacing spaces and special characters and for removing or fixing non-alphanumeric characters. In this case, it is replacing spaces with '_' and it changed the column "1_linenumber" by adding an 'x' at the beginning of it.   
...
**Task** Run the following codeblock to use the clean_names() function from janitor to standardize column names and then use rename() to change the "x1_id" column. Add a description of what this code does to your reference notebook. **Answer** This code was helpful to rename the "x1_linenumber" column in "linenumber".  
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** The Error state that there is an "error in 'summarise()'. In argument: `total_amount = sum(amount)`. Caused by error in `sum()`:
! invalid 'type' (character) of argument." Basically, it is saying that there is a problem at the beginning of the codeline caused by 'amount'. 
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **Answer** the new three letter code for amount is <dbl>. 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer** The are two with the same name: Derrick Hamilton. We can confirm they are duplicated from the 'tran-id'. 
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer** Now it has 13 rows. This code serves to clean the data. Through "clean_names()" and "rename()" we renamed the "x1_linenumber" and through mutatr(amount = as.numeric(amount)) we changed the type of data for 'amount'. Successively, distinct()" cleans up our datafram from duplicates. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** Now all the Zip codes are the same (also with the same lenght). 
...
**Task** Run the following codeblock to use str_tot_title() to standarize capitalization in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** It remains only 2 misspelled 'Conowingo', as the mistakes aren't in the capital letter use but on how they are written.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** It remains only one: 'Conowingoo'
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** Zero. str_detect() function gives the opportunity to correct every name that is not "Conowingo" in "Conowingo". 
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
