...
```{r}
voters_by_county <- read_csv("data/tabula-Eligible_Active_Voters_by_County - PP24.csv")
...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** The layout is significantly flawed. Many rows are blank or contain missing information, and actual data is being incorrectly used as column headers. For instance, "Pool Book Total" is being treated as a heading. Additionally, multiple pieces of data, like names and party affiliations, are placed within the same cell, creating confusion. This lack of clear separation between different types of information makes it difficult to interpret and analyze the data properly, as essential details are mixed up or incorrectly labeled.

```{r}
benzie_2024 <- read_csv("data/tabula-Benzie Aug 6,2024 Official Precinct Results.csv")
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** Even after removing rows with empty values, the dataset remains problematic. The columns are still unnamed, and there are no proper headers to organize the data. Furthermore, party affiliations are still combined with the candidates' names in a single cell, leading to a lack of clarity. This makes it difficult to distinguish between key details, as there is no clear separation or proper labeling for different types of information.
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** We separated the first column, renamed the first six columns for clarity, and cleaned up the candidate names. Additionally, we created a new "Office" column to categorize the candidates by their respective offices. Finally, we reordered the dataset using the select function, positioning the "Office" column at the beginning, followed by the remaining relevant data to ensure a logical and structured layout. 
