...
**Answer** Here we have a glimpse of the pg_precint dataframe. There are columns named "Shape", "shape length", and "shape area". 
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** this dataframe contains the precint id, precint names, congresses, legis, councils, schools, polling IDs, and again the shape columns, which describe multiple longitude and latitude values. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now we have the map of the area of Prince George. We have some white areas that are where there is no population living there. It shows where precints are but nothing more. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** After loading tigris in the console and running this code, it says "To enable caching of data, set `options(tigris_use_cache = TRUE)` in your R script or .Rprofile". 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Thanks to this code we now have the IDs of every single county of the U.S., including its "geometry".  
...
**Task** Run the following code. Describe the output in the space below. **Answer** We filtered to only have the Maryland counties. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is the Maryland county map. However, it has some problems such as  we can't see the bay.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** Through this code we have overload the precints of Prince George's county to the Prince George's county map that we have through the code used below. Hence, we we have the precints in PG county shown on a map.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now have a dataframe that includes the total counts for polls, early votes, absentee ballots, provisional ballots, and eligible voters in Prince George's County.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now have a dataframe that combines the PG precints with its turnout. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We got a map of Prince George's county with the number of eligible voters in each county distinguished by colors.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** With this code we have all the voters for every Maryland county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We have the population in every Maryland County from 2022. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Same data of before with the addition of the column geometry (so the possibility to create map with this dataframe). 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We want to join the md county populations with the total voters by Maryland county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Through this code we cleaned the md_county_population dataframe.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** Here we joined the two dataframes (md_county_population and voters_by_county). It has a problem: Baltimore and St Mary's compare with no values. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Finally we have good and clean joined dataframe.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now add the column "voters_per_10k". 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This map reveals that the Eastern Shore received the highest number of votes per 10,000 inhabitants, while Prince George's and Montgomery counties recorded the lowest.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Thanks to scale_fill_viridis_b(option="magma") now the difference between counties is easier to see.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** log applies to it a logarithmic scale. However, in this case, it doesn’t produce any noticeable difference.
...
**Task** Run the following code. Describe the output in the space below. **Answer** In this dataframe we now have the UNA voters per 10k in every county. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer** This map reveals a pattern similar to the earlier one, though with some notable differences. For example, Baltimore City shows a significantly lower number of unaffiliated (UNA) voters per 10,000 inhabitants compared to its previously higher turnout for general voters. Additionally, large counties like Prince George's display a decrease, with relatively low UNA voter numbers. In contrast, counties such as Frederick and Anne Arundel report a high number of UNA voters per 10,000 residents.
```{r}
ggplot() +
