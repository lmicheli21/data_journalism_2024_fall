...
**Task** Run the following code and describe the dataframe it outputs. **Answer** Ben Cardin's articles. with date, title, urle, text.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The unique() function extracts distinct elements removing any duplicates. However, it counts "dog" twice. 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** The result displays each unique element in a vertical list. Additionally, some elements, like "d", are not actual words.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** The graph reveals that the most frequently used words are articles (such as "the") or particles.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** The anti_join function performs an exclusive join. In this case, it allowed us to filter out the top 10 stop words, effectively removing unnecessary words from the data-set. 
...
**Task** Run the following code and describe the results. **Answer** In the 2022 dataframe, 'Health' appears at the top, whereas in 2024, 'Baltimore' takes the lead, likely due to the impact of the bridge accident. 'Support' consistently ranks second in both years, highlighting its ongoing importance. Additionally, funding has seen an increase in 2024 while 'Health' has fallen.
...
**Task** Run the following code and describe the results. **Answer** the top ngrams portrayed in this databse are "amount included", "health care", "prince george's", "covid 19", and "baltimore city". Interestingly, "amount included" has been used much more times than "mental health", while prince george county has also been named several times as "george's county". 
...
**Task** Run the following code and describe the results. **Answer** Compared to the 2022 dataset, the 2024 data highlights health care, amount awarded, purpose funds, Chesapeake Bay, human rights, and then Baltimore City in the top categories. There are not many significant changes, except that COVID-19 is no longer a prominent factor.
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer**  The stemming results didn't group the drive-related words together, as each variation was treated as a separate word. In contrast, lemmatization grouped most of them under the base form 'drive,' with the exception of 'driver,' which remained distinct
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer**The two datasets differ from one another. In the first, a variety of words were collected, with terms like 'community,' 'support,' and 'include' being among the most frequent. In contrast, the second dataset highlighted terms such as 'millions,' 'fund,' and 'funding' as more prominent.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** Each word in the datasets is categorized with either a positive or negative emotion. In my opinion, most of the words align with their respective emotional labels. However, there are a few cases where words, such as 'ambiguity,' which is labeled as 'negative,' don’t seem to clearly fit into either emotional category. 
