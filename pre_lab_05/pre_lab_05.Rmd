...
**Task** Combine the three files into a single data frame using the bind_rows() function, along with list(). Add a description of what this code does to your reference notebook. **Answer** It is used to combine multiple data frames (row-wise) by stacking them on top of each other.
...
**Task** Load the Maryland 2020 county population data **Answer** I have done it. 
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** We have 24 and we want 24. 
...
**Task** Examine the combined data. We have the same results but in a table. 
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** Now we can see how many people are registered voters in one specific county. The message below says that the system recognizes by itself that the matched column within the two data frame is "COUNTY". 
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** The county with the highest 18+ black population as percentage of all population 18 is Prince George's. It also coincides with the county with the highest percentage of Democrats. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** The County with the highest 18+ Asian population as a percentage of all population 18+ is Howard. The democrats percentage in there is 52.4%
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** it's considered as a number. However, Zip Codes are not numbers. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** Now the datatype is "character".  
